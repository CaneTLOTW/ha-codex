...
class MobileKeyboardAvoidanceTests(unittest.TestCase):
    def test_touch_keyboard_avoidance_stays_in_canonical_ttyd_patch(self):
        ...
        self.assertIn("viewport.removeEventListener('resize', schedule)", patch)
        self.assertIn("document.addEventListener('focusin', onFocusIn, true)", patch)
        self.assertIn("document.removeEventListener('focusin', onFocusIn, true)", patch)
        self.assertIn("xterm-helper-textarea", patch)
        self.assertIn("host.style.height = `${Math.round(hostHeight)}px`", patch)
        self.assertIn("host.style.height = '';", patch)
        ...
        self.assertIn("this.captureKeyboardBaseline(host)", patch)
        self.assertIn("componentDidMount()", patch)
        self.assertIn("componentWillUnmount()", patch)
