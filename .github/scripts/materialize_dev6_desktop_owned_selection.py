...
methods = r'''    private installDesktopShiftSelectionScroll() {
    ...
    private desktopBufferPoint(screenElement: HTMLElement, clientX: number, clientY: number) {
        ...
        if (bounds.width <= 0 || bounds.height <= 0 || this.terminal.cols <= 0 || this.terminal.rows <= 0) return undefined;
        ...
        const col = Math.min(this.terminal.cols - 1, Math.max(0, Math.floor((relativeX / bounds.width) * this.terminal.cols)));
